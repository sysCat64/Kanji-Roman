...
最初のMVPでは、部品位置の厳密判定ではなく、Unicodeの部首情報を使った「部首検索」を中心にする。将来的に、IDSデータを使って「魚 + 春」「Fish + 春 component」のような構成要素検索も追加できる設計にする。
...
`tools/json-generator/` は生成ツールの設定、入力、内部出力、検証を扱う。`data/` は静的公開サイトが直接読む完成済みJSONだけを置く。公開サイトUIと読込状態の設計は `docs/PLAN.md` の責務とし、この文書では生成ツールから公開JSONを出力する契約だけを扱う。
...
`tools/json-generator/outputs/internal/fish.json`
...
`tools/json-generator/curation/fish.json`
...
| `--reviewed-only` | 公開サイト用JSONでは `curationStatus: "reviewed"` のitemだけを出力 |
| `--sort jis` | JIS区点順でソート |
...
```mermaid
flowchart TD
  A["Start"] --> B["Load tools/json-generator/config/radicals.json"]
  ...
  I --> J["Validate output"]
  J --> K["Write internal JSON"]
  K --> L["Write data/site-index.json and data/radicals/*.json"]
```
...
- `tools/json-generator/config/radicals.json`を導入
- `fish`, `bird`, `thread`, `grass`, `tree`などを生成
- `--all`対応

### Phase 3: キュレーション対応

- `tools/json-generator/curation/*.json`を読み込む
...
- IDSによる構成要素検索
- `魚 + 春`などの自動推定
- 外部辞書・IDS由来データのライセンス確認後の追加
- CSVなどJSON以外のレビュー補助出力
